...
AUV ZIT6 是一款基于 STM32H7 系列微控制器的自主水下航行器（Autonomous Underwater Vehicle, AUV）固件系统。项目采用现代 C++17 模块化架构设计，实现了 6-DOF（六自由度）级联控制，支持位置环、速度环和直接推力控制三种工作模式。
...
\midrule
/zit6/state/pos & NED 位置 $[x,y,z,yaw]$ & 30Hz \\
/zit6/state/vel & FRD 速度 $[vx,vy,vz,vyaw]$ & 50Hz \\
/zit6/state/thr & 归一化推力 $[Fx,Fy,Fz,Myaw]$ & 30Hz \\
...
\begin{itemize}
    \item \texttt{/zit6/cmd/setpoint} — 控制设定点（ZitSetpoint）
