 ...
 */
class Self_CalcBoard_Porting {
public:
  explicit Self_CalcBoard_Porting(
...
private:
  ...
  void recoverRx(uint32_t now_ms, uint32_t reason, bool force);
  void onRxEvent(uint16_t size);
  void processAvailable();

  static Self_CalcBoard_Porting *active_instance_;
  ...
  volatile bool rx_error_pending_ = false;
...
// Compatibility name for code that still refers to the old porting class.
using Pushrod_Porting = Self_CalcBoard_Porting;
