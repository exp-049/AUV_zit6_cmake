...
__attribute__((section(".dma_buffer"))) uint8_t
    g_ms5837_uart4_dma_buffer[auv::porting::Self_CalcBoard_Porting::kDmaBufferSize];
}

namespace auv {
namespace porting {

Self_CalcBoard_Porting *Self_CalcBoard_Porting::active_instance_ = nullptr;

Self_CalcBoard_Porting::Self_CalcBoard_Porting(
...
bool Self_CalcBoard_Porting::transmitPort(void *ctx, const uint8_t *data,
                                       uint16_t length) {
  auto *self = static_cast<Self_CalcBoard_Porting *>(ctx);
...
void Self_CalcBoard_Porting::pollPort(void *ctx) {
  if (ctx != nullptr) {
    static_cast<Self_CalcBoard_Porting *>(ctx)->poll();
  }
}

bool Self_CalcBoard_Porting::startRxPort(void *ctx) {
  return ctx != nullptr && static_cast<Self_CalcBoard_Porting *>(ctx)->startRx();
}

uint32_t Self_CalcBoard_Porting::getTickPort(void *ctx) {
  (void)ctx;
  return HAL_GetTick();
}

uint32_t Self_CalcBoard_Porting::getRxRecoveryCountPort(void *ctx) {
  if (ctx == nullptr) {
    return 0U;
  }
  return static_cast<Self_CalcBoard_Porting *>(ctx)->recovery_count_;
}

uint32_t Self_CalcBoard_Porting::getRxErrorCountPort(void *ctx) {
  return ctx != nullptr
             ? static_cast<Self_CalcBoard_Porting *>(ctx)->uart_error_count_
             : 0U;
}

uint32_t Self_CalcBoard_Porting::getLastRxErrorPort(void *ctx) {
  return ctx != nullptr
             ? static_cast<Self_CalcBoard_Porting *>(ctx)->last_uart_error_
             : 0U;
}

uint32_t Self_CalcBoard_Porting::getLastRxRecoveryReasonPort(void *ctx) {
  return ctx != nullptr
             ? static_cast<Self_CalcBoard_Porting *>(ctx)->last_recovery_reason_
             : 0U;
}

uint32_t Self_CalcBoard_Porting::getRxEventCountPort(void *ctx) {
  return ctx != nullptr
             ? static_cast<Self_CalcBoard_Porting *>(ctx)->rx_event_count_
             : 0U;
}

uint32_t Self_CalcBoard_Porting::getDmaWritePosPort(void *ctx) {
  if (ctx == nullptr) {
    return 0U;
  }
  auto *self = static_cast<Self_CalcBoard_Porting *>(ctx);
...
void Self_CalcBoard_Porting::handleHalRxEvent(UART_HandleTypeDef *huart,
                                           uint16_t size) {
...
void Self_CalcBoard_Porting::handleHalError(UART_HandleTypeDef *huart) {
  if (active_instance_ != nullptr && huart == active_instance_->huart_) {
    ++active_instance_->uart_error_count_;
    active_instance_->last_uart_error_ = huart->ErrorCode;
    // HAL clears the UART flags in the ISR, but an overrun can leave the
    // ReceiveToIdle state unusable while the DMA stream still appears active.
    // Defer the abort/restart to poll(); HAL receive APIs are not called from
    // this interrupt callback.
    active_instance_->rx_error_pending_ = true;
  }
}

bool Self_CalcBoard_Porting::startIdleDma() {
...
bool Self_CalcBoard_Porting::startRx() {
  ...
  last_event_size_ = 0U;
  rx_event_pending_ = false;
  rx_error_pending_ = false;
  return startIdleDma();
}

void Self_CalcBoard_Porting::onRxEvent(uint16_t size) {
...
bool Self_CalcBoard_Porting::dmaIsActive() const {
  if (huart_ == nullptr || huart_->hdmarx == nullptr ||
...
void Self_CalcBoard_Porting::recoverRx(uint32_t now_ms, uint32_t reason,
                                       bool force) {
  if (!force && (uint32_t)(now_ms - last_recovery_ms_) < kRecoveryIntervalMs) {
    return;
  }
...
void Self_CalcBoard_Porting::processAvailable() {
...
void Self_CalcBoard_Porting::poll() {
  if (huart_ == nullptr || huart_->hdmarx == nullptr || backend_ == nullptr) {
    return;
  }
  ...
  const bool uart_error = rx_error_pending_;
  const uint32_t uart_error_code = last_uart_error_;
  processAvailable();
  rx_event_pending_ = false;
  ...
  if (uart_error) {
    rx_error_pending_ = false;
    // Keep the HAL error code in the recovery reason with a high-bit marker;
    // reason=1 remains reserved for the normal DMA-disabled recovery path.
    recoverRx(now_ms, 0x10000U | uart_error_code, true);
    return;
  }
