...
inline Matrix6f eulerToRotationMatrix(float roll, float pitch,
  ...
  float cφ = std::cos(roll);
  float sφ = std::sin(roll);
  float cθ = std::cos(pitch);
  float sθ = std::sin(pitch);
  float cψ = std::cos(yaw);
  float sψ = std::sin(yaw);

  // ---- 左上 3×3：线速度旋转矩阵 R_ZYX (World = R · Body) ----
  auto R_rot = R.topLeftCorner<kRotDim, kRotDim>();
  R_rot(0, 0) = cψ * cθ;
  R_rot(0, 1) = cψ * sθ * sφ - sψ * cφ;
  R_rot(0, 2) = cψ * sθ * cφ + sψ * sφ;

  R_rot(1, 0) = sψ * cθ;
  R_rot(1, 1) = sψ * sθ * sφ + cψ * cφ;
  R_rot(1, 2) = sψ * sθ * cφ - cψ * sφ;
  ...
  if (std::abs(cθ) > kCosThetaEps) {
    float tθ = sθ / cθ; // tan(θ)
    R_T(0, 0) = 1.0f;
    R_T(0, 1) = sφ * tθ;
    R_T(0, 2) = cφ * tθ;
...
inline Matrix6f eulerToRotationMatrixInverse(float roll, float pitch,
  ...
  auto R_rot = R.topLeftCorner<kRotDim, kRotDim>();
  R_rot(0, 0) = cψ * cθ;
  R_rot(1, 0) = cψ * sθ * sφ - sψ * cφ;
  R_rot(2, 0) = cψ * sθ * cφ + sψ * sφ;

  R_rot(0, 1) = sψ * cθ;
  R_rot(1, 1) = sψ * sθ * sφ + cψ * cφ;
  R_rot(2, 1) = sψ * sθ * cφ - cψ * sφ;
