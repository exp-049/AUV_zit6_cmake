...
struct InsPortDiagnostics {
  ...
  uint16_t tx_last_size = 0;
  uint8_t tx_last_status = 0;
  bool tx_uart_ready = false;
};
...
public:
  ...
  bool sendCommand(uint8_t cmd_id, uint8_t value);
  bool sendCommand(uint8_t cmd_id, const uint8_t *data, uint8_t data_len);
  bool resetPosition();
  bool setDvlPower(bool on);
  bool restart();
  bool setInitialPosition(double lat, double lon);
