...
bool INS_Driver::sendCommand(uint8_t cmd_id, uint8_t value) {
  ...
  return sendCommand(cmd_id, data, 8);
}

bool INS_Driver::sendCommand(uint8_t cmd_id, const uint8_t *data,
                              uint8_t data_len) {
  ...
  for (int i = 0; i < 3; i++) {
    if (ops_.transmit(ops_.ctx, cmd, sizeof(cmd)))
...
bool INS_Driver::resetPosition() { return sendCommand(0x02, 0x00); }

bool INS_Driver::setDvlPower(bool on) {
  // 手册：03 为 DVL 电源控制位, 01 为开, 00 为关
  return sendCommand(0x03, on ? 0x01 : 0x00);
}

bool INS_Driver::restart() {
  // 手册：04 为重启指令
  return sendCommand(0x04, 0x00);
}

bool INS_Driver::setInitialPosition(double lat, double lon) {
  uint8_t data[8];
  ProtocolParser::encodeInitialPosition(lat, lon, data);
  return sendCommand(0x20, data, 8);
}
...
void INS_Driver::getDiagnostics(InsPortDiagnostics &out) const {
  ...
  if (ops_.getDiagnostics != nullptr) {
    ...
    out.tx_calls = port.tx_calls;
    out.tx_attempts = port.tx_attempts;
    out.tx_successes = port.tx_successes;
    out.tx_failures = port.tx_failures;
    out.tx_last_size = port.tx_last_size;
    out.tx_last_status = port.tx_last_status;
    out.tx_uart_ready = port.tx_uart_ready;
