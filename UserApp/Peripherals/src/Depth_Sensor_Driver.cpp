...
Depth_Sensor_Driver::Depth_Sensor_Driver(DepthBackend *backend,
                                         PushrodBackend *legacy_pushrod)
    : backend_(backend), legacy_pushrod_backend_(legacy_pushrod) {
...
bool Depth_Sensor_Driver::sendPushrodTask(
    const PushrodTask &task) {
  return legacy_pushrod_backend_ != nullptr &&
         legacy_pushrod_backend_->sendTask(task);
}

bool Depth_Sensor_Driver::sendPushrodTask(uint32_t task_id,
                                          int16_t power_x1000,
...
bool Depth_Sensor_Driver::readPushrodAck(PushrodAck *ack) {
  return legacy_pushrod_backend_ != nullptr &&
         legacy_pushrod_backend_->readAck(ack);
}
