 ...
 * - /zit6/sim/pos      — SITL 仿真位置（6DOF）
 * - /zit6/sim/vel      — SITL 仿真速度（6DOF）
 */
class MicroRosSubscriber {
public:
  MicroRosSubscriber() = default;
...
private:
  ...
  std_msgs__msg__Float32MultiArray sim_pos_msg_, sim_vel_msg_;

  // SITL 6 元素缓冲区
  float sim_pos_buf_[6] = {0};
  float sim_vel_buf_[6] = {0};
