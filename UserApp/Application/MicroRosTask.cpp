    ...
    if (state == WAITING_AGENT) {
      if (RCL_RET_OK == rmw_uros_ping_agent(200, 1)) {
        if (RCL_RET_OK ==
          ...
          // --- USBL 数据发布 (Float32MultiArray: [roll, pitch, yaw, beacon_north, beacon_east, beacon_depth, energy[0..7]]) ---
          std_msgs__msg__Float32MultiArray__init(&usbl_fb_msg_);
          usbl_fb_msg_.data.data = usbl_buf_;
          usbl_fb_msg_.data.size = 14;
          usbl_fb_msg_.data.capacity = 14;
          rclc_publisher_init_default(
              &usbl_pub_, &node_,
              ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Float32MultiArray),
              "/zit6/state/usbl");
      ...
      if (RCL_RET_OK != rmw_uros_ping_agent(500, 3)) {
        ...
        // --- USBL 数据发布 (33Hz) ---
        {
          static uint32_t last_usbl_pub_tick = 0;
          if (now_ms - last_usbl_pub_tick >= 30) {
            last_usbl_pub_tick = now_ms;
            auv::UsblState usbl_snapshot;
            taskENTER_CRITICAL();
            usbl_snapshot = shared_usbl_state;
            taskEXIT_CRITICAL();

            usbl_buf_[0] = usbl_snapshot.roll;
            usbl_buf_[1] = usbl_snapshot.pitch;
            usbl_buf_[2] = usbl_snapshot.yaw;
            usbl_buf_[3] = usbl_snapshot.beacon_north;
            usbl_buf_[4] = usbl_snapshot.beacon_east;
            usbl_buf_[5] = usbl_snapshot.beacon_depth;
            usbl_buf_[6] = usbl_snapshot.energy[0];
            usbl_buf_[7] = usbl_snapshot.energy[1];
            usbl_buf_[8] = usbl_snapshot.energy[2];
            usbl_buf_[9] = usbl_snapshot.energy[3];
            usbl_buf_[10] = usbl_snapshot.energy[4];
            usbl_buf_[11] = usbl_snapshot.energy[5];
            usbl_buf_[12] = usbl_snapshot.energy[6];
            usbl_buf_[13] = usbl_snapshot.energy[7];
            usbl_fb_msg_.data.size = 14;
            rcl_publish(&usbl_pub_, &usbl_fb_msg_, NULL);
          }
        }
