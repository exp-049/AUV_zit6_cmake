...
class HitlSimulator {
public:
  HitlSimulator(float dt = 0.01f) : dt_(dt) {
  ...
  void step(const std::array<float, 6> &forces) {
    ...
    for (int i = 0; i < 6; i++) {
      ...
      float restore = 0.0f;
      if (i == 3)
        restore = -weight * sim.metacentric_height * std::sin(position_[3]) *
                  std::cos(position_[4]);
      if (i == 4)
        restore = -weight * sim.metacentric_height * std::cos(position_[3]) *
                  std::sin(position_[4]);
    ...
    // 2. 速度积分
    for (int i = 0; i < 6; i++)
      velocity_[i] += accel[i] * dt_;

    // 3. 运动学更新：η̇ = R(η) · ν
    auv::algorithm::math::applyRotationToWorld(velocity_.data(),
                                               eta_dot_.data(), position_[3],
                                               position_[4], position_[5]);
    for (int i = 0; i < 6; i++)
      position_[i] += eta_dot_[i] * dt_;

    // 角度归一化
    for (int i = 3; i < 6; i++) {
      if (position_[i] > auv::algorithm::math::kPi)
        position_[i] -= auv::algorithm::math::kTwoPi;
      else if (position_[i] < -auv::algorithm::math::kPi)
        position_[i] += auv::algorithm::math::kTwoPi;
    }
  }
