...
\subsection{Depth\_Sensor\_Driver（深度传感器）}

支持商品 UART、自研 UART、M14 UART 和 I2C 四种后端，提供深度和温度测量；推杆由独立的 Pushrod\_Driver 管理。
...
\begin{itemize}
    ...
    \item \textbf{Depth\_Sensor\_Porting}：统一选择并组装深度后端
    \item \textbf{Pushrod\_Porting}：自研解算板 UART4 DMA 和推杆/深度共享链路
\end{itemize}
