...
- 当前上位机主控协议仍是 4-DOF：`[X, Y, Z, Yaw]`
- 控制器内部已经扩展为 6-DOF 状态和输出
...
示例：
- `type_mask = 0`：更新 X/Y/Z/Yaw 全部四个轴
- `type_mask = 8`：只跳过 Yaw，更新 X/Y/Z
- `type_mask = 14`：只更新 X，保持 Y/Z/Yaw 不变
...
- 当前发布频率：约 `30Hz`
- 数据格式：`[fx, fy, fz, myaw]`
- 含义：4-DOF 输出快照，其中 `myaw` 实际来自内部 `forces[5]`
...
5. 满足条件后：
   ...
   - 清空当前 setpoint
   - 设置 `is_armed = true`
6. 解锁后，若 500ms 内未收到心跳，则自动上锁并切回 `NONE`
...
```bash
ros2 topic pub /zit6/cmd/setpoint zit6_interfaces/msg/ZitSetpoint '{control_key: 0, type_mask: 8, x: 1.0, y: 0.0, z: -1.0, yaw: 0.0, seq: 1}'
```

这里 `type_mask = 8` 表示跳过 Yaw，只更新 X/Y/Z。

### 7.4 机体系速度控制：只控制 X 轴前进速度

```bash
ros2 topic pub /zit6/cmd/setpoint zit6_interfaces/msg/ZitSetpoint '{control_key: 17, type_mask: 14, x: 0.2, y: 0.0, z: 0.0, yaw: 0.0, seq: 2}'
```

解释：
- `17 = 0x10 | 0x01`：Body + Velocity
- `14 = 0b1110`：仅更新 X，跳过 Y/Z/Yaw
